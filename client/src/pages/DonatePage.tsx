...
export const DonatePage = (): JSX.Element => {
  ...
  const donateMutation = useMutation({
    mutationFn: async (amountValue: number) => {
      await apiRequest("POST", "/api/donations", { amount: amountValue });
    },
    onSuccess: () => {
      ...
      toast({
        ...
        description: `Your contribution helps protect our reefs, ${form.firstName || "friend"}.`,
      });
      setAmount("");
      setForm({
        firstName: "",
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const numeric = Math.round(Number(amount.replace(/[^0-9.]/g, "")));
